...
<head>
  ...
  <title>Coral Cool Aid — Couture Corals</title>
  <meta name="description" content="Coral Cool Aid — sustainably grown couture corals. Photo-true listings and a neon reef aesthetic."/>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .btn.primary{
      border-color: rgba(56,227,255,.35);
      background: linear-gradient(135deg, rgba(56,227,255,.18), rgba(255,70,197,.12));
    }

    #reefCanvas{position:fixed; inset:0; z-index:0; pointer-events:none; opacity:.45;}
    ...
    .card::before{
      ...
      background:
        radial-gradient(600px 220px at 40% 0%, rgba(56,227,255,.22), transparent 60%),
        radial-gradient(520px 240px at 90% 30%, rgba(255,70,197,.18), transparent 55%);
      pointer-events:none;
      opacity:.85;
    }
    ...
    .title{margin:14px 0 10px; font-size:42px; line-height:1.05; letter-spacing:-.8px;}
    .subtitle{margin:0; font-size:15px; color:var(--muted); max-width:54ch; line-height:1.45}
    .heroCtas{margin-top:16px; display:flex; gap:10px; flex-wrap:wrap}
    .quickFacts{margin-top:18px; display:grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap:10px;}
    .fact{padding:12px; border-radius:14px; background: rgba(255,255,255,.06); border:1px solid rgba(255,255,255,.10);}
    ...
    video{width:100%; display:block; aspect-ratio: 16/9; object-fit: cover; filter: saturate(1.08) contrast(1.02);}
    .videoLabel{
    ...
    .product:hover{transform: translateY(-3px); border-color: rgba(56,227,255,.28); background: rgba(255,255,255,.08);}
    .thumb{width:100%; height:170px; object-fit: cover; display:block; filter: saturate(1.1); transition: transform .22s ease;}
    .product:hover .thumb{transform: scale(1.03)}
    .pbody{padding:12px 12px 14px}
    .pname{margin:0; font-size:14px; font-weight:900; letter-spacing:.2px}
    .ptag{margin:6px 0 0; font-size:12px; color:var(--muted)}
    .badge{
    ...
    .modalBack{position:fixed; inset:0; display:none; place-items:center; background: rgba(0,0,0,.55); z-index:50; padding:16px;}
    .modal{
...
<body>
  ...
  <main class="wrap">
    <section class="hero">
      <div class="card">
        <div class="heroText">
          <div class="kicker">✨ Neon reef vibes • Premium frags</div>
          <h2 class="title">Coral that pops under blues.</h2>
          <p class="subtitle">
            Coral Cool Aid curates standout, aquacultured pieces—grown clean, photographed honestly, and ready for your reef.
          </p>
          ...
          <div class="quickFacts">
            <div class="fact"><strong>Sustainably grown</strong><span>Local propagation focus</span></div>
            <div class="fact"><strong>Photo-true</strong><span>Same-piece listings</span></div>
            <div class="fact"><strong>Reef-friendly</strong><span>Care guidance included</span></div>
          </div>
        </div>
      </div>
    ...
    <footer>
      ...
      <div>DM for availability • Local pickup / shipping options</div>
    </footer>
  </main>
  ...
  <script>
    ...
    const bubbles = Array.from({length: 42}).map(()=>({
      x: Math.random()*w, y: Math.random()*h,
      r: (6 + Math.random()*22)*dpr,
      vy: (0.25 + Math.random()*0.9)*dpr,
    ...
    function step(){
      ...
      const g = ctx.createRadialGradient(pointer.x, pointer.y, 0, pointer.x, pointer.y, 280*dpr);
      g.addColorStop(0, 'rgba(56,227,255,.20)');
      g.addColorStop(0.6, 'rgba(255,70,197,.10)');
      ...
      for(const b of bubbles){
        ...
        grad.addColorStop(0.55, `rgba(56,227,255,${b.a*0.65})`);
